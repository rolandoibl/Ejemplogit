...
def evaluate(weights, image):
    ...
    #    
    return 1/(1+numpy.exp(numpy.dot(-weights, image)))
...
def train_perceptron(weights, images, labels, desired_digit):
    ...
    attempts = 2000                                    #Max number of training iterations
    ...
    #     attempts = attempts - 1
    #
    while(np.linalg.norm(gradient) > tol and attempts > 0 and not rospy.is_shutdown()):
        gradient = numpy.zeros(len(weights))
        a = 0
        for x in inputs:
            y_hat = evaluate(weights, x)
            y = 1 if labels[a] == desired_digit else 0
            g_j = x*(y_hat - y)*(y_hat*(1 - y_hat))
            gradient = numpy.add(gradient, g_j)
            a += 1
